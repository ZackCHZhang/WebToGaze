...
class GAZEMININGWITHMASK:
    ...
    def __init__(self, data_path):
        ...
        self._dir_stimuli_train = data_path + "Training/data"
        self._dir_stimuli_valid = data_path + "Val/data"
        self._dir_imgview_mask = data_path + "Training/mask"

        self._dir_saliency_train = data_path + "Training/label"
        self._dir_saliency_valid = data_path + "Val/label"
        self._dir_imgview_valid = data_path + "Val/mask"


    def load_data(self):
        train_list_x = _get_file_list(self._dir_stimuli_train)
        train_list_y = _get_gaze_file_list(self._dir_saliency_train)
        train_mask_list = _get_mask_list(self._dir_imgview_mask)

        # _check_consistency(zip(train_list_x, train_list_y), 471)

        train_set = _fetch_dataset_with_mask((train_list_x, train_mask_list, train_list_y),
                                   self._target_size, True)

        valid_list_x = _get_file_list(self._dir_stimuli_valid)
        valid_list_y = _get_gaze_file_list(self._dir_saliency_valid)
        valid_mask_list = _get_mask_list(self._dir_imgview_valid)
        # print(len(valid_list_x))
        # print(len(valid_list_y))

        # _check_consistency(zip(valid_list_x, valid_list_y), 94)

        valid_set = _fetch_dataset_with_mask((valid_list_x, valid_mask_list, valid_list_y),
                                   self._target_size, False)

        return (train_set, valid_set)


class GAZEMININGWITHDOUBLEMASK:
    """
    """
    ...
    def __init__(self, data_path):
        ...
        self._dir_stimuli_valid = data_path + "Val/data"
        self._dir_imgview_mask = data_path + "Training/mask"
        self._dir_textview_mask = data_path + "Training/text_mask"

        self._dir_saliency_train = data_path + "Training/label"
        self._dir_saliency_valid = data_path + "Val/label"
        self._dir_imgview_valid = data_path + "Val/mask"
        self._dir_textview_valid = data_path + "Val/text_mask"
    ...
    def load_data(self):
        ...
        train_list_y = _get_gaze_file_list(self._dir_saliency_train)
        train_mask_list = _get_mask_list(self._dir_imgview_mask)
        train_textmask_list = _get_mask_list(self._dir_textview_mask)

        # _check_consistency(zip(train_list_x, train_list_y), 471)

        train_set = _fetch_dataset_with_doublemask((train_list_x, train_mask_list, train_textmask_list, train_list_y),
                                   self._target_size, True)

        valid_list_x = _get_file_list(self._dir_stimuli_valid)
        valid_list_y = _get_gaze_file_list(self._dir_saliency_valid)
        valid_mask_list = _get_mask_list(self._dir_imgview_valid)
        valid_textmask_list = _get_mask_list(self._dir_textview_valid)
        # print(len(valid_list_x))
        # print(len(valid_list_y))

        # _check_consistency(zip(valid_list_x, valid_list_y), 94)

        valid_set = _fetch_dataset_with_doublemask((valid_list_x, valid_mask_list, valid_textmask_list, valid_list_y),
                                   self._target_size, False)

        return (train_set, valid_set)

class ALLDATASETWITHDOUBLEMASK:
    """
    """
...
class EYEVEDOWITHMASK:
    ...
    def __init__(self, data_path):
        ...
        self._dir_stimuli_train = data_path + "Training/data"
        self._dir_stimuli_valid = data_path + "Val/data"
        self._dir_imgview_mask = data_path + "Training/text_mask"

        self._dir_saliency_train = data_path + "Training/label"
        self._dir_saliency_valid = data_path + "Val/label"
        self._dir_imgview_valid = data_path + "Val/text_mask"
...
class SALICON:
    ...
    def load_data(self):
        ...
        _check_consistency(zip(train_list_x, train_list_y), 9999)
...
class TESTWITHDOUBLEMASK:
    ...
    def __init__(self, dataset, data_path):
        ...
        self._dir_stimuli_test = data_path + '/data/'
        self._dir_mask_test = data_path + '/mask/'
        self._dir_textmask_test = data_path + '/text_mask/'

    def load_data(self):
        test_list_x = _get_file_list(self._dir_stimuli_test)
        # print(test_list_x)
        test_list_mask = _get_file_list(self._dir_mask_test)
        test_list_textmask = _get_file_list(self._dir_textmask_test)
        test_set = _fetch_dataset_with_doublemask((test_list_x, test_list_textmask, test_list_mask), self._target_size,
                                  False, online=True)
        return test_set
...
class TESTWITHMASK:
    ...
    def __init__(self, dataset, data_path):
        ...
        self._dir_stimuli_test = data_path + '/data/'
        self._dir_mask_test = data_path + '/text_mask/'

    def load_data(self):
        test_list_x = _get_file_list(self._dir_stimuli_test)
        # print(test_list_x)
        test_list_mask = _get_file_list(self._dir_mask_test)
        test_set = _fetch_dataset_with_mask((test_list_x, test_list_mask), self._target_size,
                                  False, online=True)
        return test_set
...
def get_dataset_iterator(phase, dataset, data_path):
    ...
    if phase == "test":
        test_class = TESTWITHDOUBLEMASK(dataset, data_path)
...
def _fetch_dataset_with_mask(files, target_size, shuffle, online=False):
    ...
    dataset = dataset.map(lambda *files: _parse_function_with_mask(files, target_size),
    ...
    # dataset = dataset.repeat(config.PARAMS['n_epochs'])

    return dataset


def _fetch_dataset_with_doublemask(files, target_size, shuffle, online=False):
    """Here the list of file directories is shuffled (only when training),
       loaded, batched, and prefetched to ensure high GPU utilization.
    ...
    dataset = dataset.map(lambda *files: _parse_function_with_doublemask(files, target_size),
                          num_parallel_calls=tf.data.experimental.AUTOTUNE)
...
def _parse_function_with_doublemask(files, target_size):
    ...
    # print(files)
    tarin_data = tf.concat([image_list[0], image_list[1], image_list[2]], axis=2)
    image_list.insert(0,tarin_data)
    image_list.pop(1)
    image_list.pop(1)
    image_list.pop(1)
    print(image_list)
...
def _parse_function_with_mask(files, target_size):
    """This function reads image data dependent on the image type and
       whether it constitutes a stimulus or saliency map. All instances
       are then reshaped and padded to yield the target dimensionality.
    ...
    image_list.append(files)
    # print(files)
    tarin_data = tf.concat([image_list[0], image_list[1]], axis=2)
    image_list.insert(0,tarin_data)
    image_list.pop(1)
    image_list.pop(1)
    print(image_list)
...
def _get_gaze_file_list(data_path):
    ...
    if os.path.isfile(data_path):
        ...
        for subdir, dirs, files in os.walk(data_path):
            for file in files:
                if file.lower().endswith(("_pureheat.png")):
...
def _check_consistency(zipped_file_lists, n_total_files):
    ...
    print(len(list(zipped_file_lists)))
