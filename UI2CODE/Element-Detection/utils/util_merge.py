...
def draw_mask(org, compos, color_map=C.COLOR, line=2, show=False, name='img', mask_class='ImageView'):
    class_colors = color_map
    canvas = np.zeros((org.shape[0], org.shape[1]), np.uint8)
    back_area = org.shape[0] * org.shape[1]
    for compo in compos:
        if compo.category == mask_class and compo.area *2 < back_area:
            corner = compo.put_bbox()
            canvas = cv2.rectangle(canvas, (corner[0], corner[1]), (corner[2], corner[3]), class_colors[compo.category], thickness=-1)
    if show:
